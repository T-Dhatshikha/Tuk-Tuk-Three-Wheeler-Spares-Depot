...
public class POSController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        qtyCol.setCellValueFactory(
                data -> new javafx.beans.property.SimpleStringProperty(
                        String.valueOf(
                                data.getValue().getBuyingQuantity())));

        subtotalCol.setCellValueFactory(
                data -> new javafx.beans.property.SimpleStringProperty(
                        "Rs." + String.format("%.2f",
                                data.getValue().discountedSubtotal())));

        discountCol.setCellValueFactory(
                data -> new javafx.beans.property.SimpleStringProperty(
                        data.getValue().bulkDiscount()
                                ? "5% OFF" : "-"));
    ...
    private void addToCart(ActionEvent event) {
        ...
        Spares spare = InventoryController.inventory.searchByCode.(code);
    ...
    private void refreshCart() {
        ...
        rawTotalLabel.setText(
                "Raw Total: Rs."
                        + String.format("%.2f", cart.rawTotal()));
        ...
        if (cart.hasSynergyDiscount()) {
            ...
            discountLabel.setText(
                    "Bulk discount: 5% off items with qty 3+");
            discountLabel.setStyle(
                    "-fx-text-fill: black;");
        }
        ...
        finalTotalLabel.setText(
                "FINAL TOTAL: Rs."
                        + String.format("%.2f", cart.finalTotal()));
    }
